...
import com.ydl.database.datasource.BaseDatabaseConfiguration;
 ...
 * 6. SlaveDatabaseAutoConfiguration 中的 basePackages = {"com.ydl.pinda"}, 修改为  basePackages = {"com.ydl.pinda.order.dao"} # 这个路径根据你的情况修改
 * 7. mysql.yml 中增加2个数据源的配置 spring.database.druid.master 和 spring.database.druid.slave
 * <p>
 * 完成上述修改后， 位于com.ydl.pinda.product.dao 包下的dao 将操作  master 库， com.ydl.pinda.order.dao中的dao 将操作slave库
 */
@Configuration
@Slf4j
@MapperScan(
        basePackages = {"cn.jinronga.sms",},
        annotationClass = Repository.class,
        sqlSessionFactoryRef = SmsServerDatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusProperties.class, DatabaseProperties.class})
public class SmsServerDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    /**
     * 每个数据源配置不同即可
     */
    final static String DATABASE_PREFIX = "master";

    public SmsServerDatabaseAutoConfiguration(MybatisPlusProperties properties,
